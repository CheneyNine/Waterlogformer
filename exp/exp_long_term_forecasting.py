...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def save_model_and_input(self, example_input, setting):
        ...
        batch_x, future_rain_data, batch_x_mark, dec_inp, batch_y_mark = example_input  # 假设 example_input 是一个包含 (batch_x, future_rain_data) 的元组
        ...
        batch_x_mark = batch_x_mark.to("cpu")  # 确保 future_rain_data 在 CPU 上
        dec_inp = dec_inp.to("cpu")  # 确保 future_rain_data 在 CPU 上
        batch_y_mark = batch_y_mark.to("cpu")  # 确保 future_rain_data 在 CPU 上
    ...
    def _single_mask_contrastive_loss(self, sim_matrix, pos_mask, temperature):
        """
        单个掩码对应的 InfoNCE 损失（用于 static 或 dynamic）
        """
        sim = sim_matrix / temperature
        sim = sim - sim.logsumexp(dim=1, keepdim=True)  # log-softmax
        log_prob = sim

        # 负样本掩码：高相似但非正样本
        hard_neg_mask = (sim_matrix > 0.4) & (~pos_mask)
    ...
    def contrastive_loss(self, reprs, near_mask, same_tpi_mask, epoch, max_epochs=15, temperature=0.07):
        """
        对比损失函数（适用于单个样本的节点表征，非 batch 版本）
        
        Args:
            reprs: [N, D] 节点表征
            near_mask: [N, N] 静态邻接正样本掩码
            same_tpi_mask: [N, N] 同类型动态正样本掩码（样本相关）
            epoch: 当前轮数
            max_epochs: 总训练轮数（用于动态权重）
            temperature: 温度系数
        Returns:
            scalar 对比损失
        """
        ...
        sim_matrix = F.cosine_similarity(
            reprs.unsqueeze(1),  # [N, 1, D]
            reprs.unsqueeze(0),  # [1, N, D]
            dim=-1               # -> [N, N]
        )

        # --- Step 2: 构造 mask ---
        alpha = 1.0 - min(epoch / (max_epochs * 0.3), 1.0)  # 前 30% 过渡
        near_mask = near_mask.bool().to(reprs.device)       # [N, N]
        same_tpi_mask = same_tpi_mask.bool().to(reprs.device)  # [N, N]
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,future_rain_data) in enumerate(vali_loader):
                ...
                dec_inp = torch.cat([dec_inp_known, dec_inp_rand], dim=2)       # shape: [B, N, L_label + L_pred, D]
                # encoder - decoder
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
                else:
                     # 取前7个切片并在 dim=1 上取均值，得到 (B, 24, 8)
                    mean_slice = batch_x[:, :7].mean(dim=1, keepdim=True)  # (B, 1, 24, 8)
                    # 取余下的切片 (从第7到63)
                    remaining = batch_x[:, 7:]  # (B, 56, 24, 8)
                    # 拼接得到 (B, 1+56=57, 24, 8)
                    batch_x= torch.cat((mean_slice, remaining), dim=1)
                    
                    mean_slice = future_rain_data[:, :7].mean(dim=1, keepdim=True)  # (B, 1, 24, 8)
                    # 取余下的切片 (从第7到63)
                    remaining = future_rain_data[:, 7:]  # (B, 56, 24, 8)
                    # 拼接得到 (B, 1+56=57, 24, 8)
                    future_rain_data= torch.cat((mean_slice, remaining), dim=1)

                    mean_slice = dec_inp[:, :7].mean(dim=1, keepdim=True)  # (B, 1, 24, 8)
                    # 取余下的切片 (从第7到63)
                    remaining = dec_inp[:, 7:]  # (B, 56, 24, 8)
                    # 拼接得到 (B, 1+56=57, 24, 8)
                    ...
                    # outputs,contrastive_loss = self.model(batch_x, future_rain_data, None, dec_inp, None)
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,future_rain_data) in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    ...
                    distance_matrix = np.load('/root/Time-Series-Library-main/dataset/distance_matrix_km.npy')
                    distance_matrix = torch.tensor(distance_matrix, dtype=torch.float32).to(batch_x.device)
                    slope_corr_matrix = np.load('/root/Time-Series-Library-main/dataset/slope_correlation_matrix.npy')
                    ...
                    # print("same_tpi_mask",same_tpi_mask.shape)
    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,future_rain_data) in enumerate(test_loader):
                ...
                if self.args.use_amp:
                    ...
                    print(batch_x.shape, batch_x_mark.shape, dec_inp.shape, batch_y_mark.shape,future_rain_data.shape)
        ...
        example_input = (batch_x, future_rain_data, batch_x_mark, dec_inp, batch_y_mark)  # 创建一个包含两个输入的元组
        self.save_model_and_input(example_input, setting)

        # 保存为 .npy 文件
        batch_x_np = batch_x.cpu().numpy()  # 移动到 CPU 并转换为 numpy
        future_rain_data_np = future_rain_data.cpu().numpy()  # 移动到 CPU 并转换为 numpy
        ...
        for i in range(preds.shape[0]):  # 遍历 batch
            for j in range(preds.shape[1]):  # 遍历每个节点
                true_seq = trues[i, j, :]  # [24]
                pred_seq = preds[i, j, :]  # [24]
