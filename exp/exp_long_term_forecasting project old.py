...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def test(self, setting, test=0):
        ...
        print('mse:{:.4f}, mae:{:.4f}, dtw:{}'.format(mse, mae, dtw))
        f = open("result_long_term_forecast.txt", 'a')
        f.write(setting + "  \n")
        f.write('mse:{}, mae:{}, dtw:{}'.format(mse, mae, dtw))
        f.write('\n')
        ...
        spike_count = 0
        for i in range(preds.shape[0]):
            true_sample = trues[i, :, 0]
            pred_sample = preds[i, :, 0]
            if is_spike(true_sample):
                spike_count += 1
                mae_i = np.mean(np.abs(true_sample - pred_sample))
                mse_i = np.mean((true_sample - pred_sample) ** 2)
                spike_mae_list.append(mae_i)
                spike_mse_list.append(mse_i)
        if spike_count > 0:
            print(f"📈 激增样本数量: {spike_count} / {len(preds)}")
