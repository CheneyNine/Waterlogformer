        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,future_rain_data) in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    ...
                    # # 对比学习 contrastive losses using encoder representations
                    # # Use the model’s registered buffer for distance matrix
                    # distance_matrix = self.model.distance_matrix.to(self.device)
                    # near_mask = distance_matrix[7:, 7:] < 3.0
                    # tpi = batch_x[..., -1].long()
                    ...
                    # # Use the model’s registered buffer for slope correlation
                    # strong_corr_mask = (self.model.slope_corr_matrix.to(self.device)[-56:, -56:] > 0)
                    ...
                    # 输出：batch_y 和 outputs 是 [32, 63, 24]
                    loss = criterion(outputs, batch_y) + self.args.lambda_contrast* contrastive_loss+loss_sharpness * self.args.sharpness
                    train_loss.append(loss.item())
