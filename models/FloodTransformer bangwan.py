    ...
    def forward(self, x_enc, future_rain, x_mark_enc=None, x_dec=None, x_mark_dec=None):
        ...
        if self.use_future_rain:
            ...
            if self.number_fuse:
                ...
                future_rain_embed = self.future_rain_embedding(rain_pred_input, x_mark_future_rain)
                future_rain_encoded, _ = self.future_rain_encoder(future_rain_embed)
                future_rain_encoded=self.compute_rainfall_influence(future_rain_encoded.reshape(batch_size,7,seq_len,-1),adj, self.alpha3, self.beta3)
            ...
            for layer in self.flood_decoder_layers_future_rain:
                dec_f = layer(dec_f, future_rain_encoded, static_s_dec,enc_out_flood, self_mask=None, cross_mask=None)
        else:
            dec_f = dec_out_flood 
