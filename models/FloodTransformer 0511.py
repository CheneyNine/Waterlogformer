...
class Model(nn.Module):
    def __init__(self,configs):
        ...
        # Embedding
        self.enc_embedding_rain = DataEmbedding(configs.enc_in, configs.d_model, configs.embed, configs.freq,
                                           configs.dropout)
        self.enc_embedding_flood = DataEmbedding(configs.enc_in, configs.d_model, configs.embed, configs.freq,
                                           configs.dropout)
        self.dec_embedding_rain = DataEmbedding(configs.dec_in, configs.d_model, configs.embed, configs.freq,
                                               configs.dropout)
        self.dec_embedding_flood = DataEmbedding(configs.dec_in, configs.d_model, configs.embed, configs.freq,
                                               configs.dropout)
    ...
    def forward(self, x_enc, future_rain, x_mark_enc=None, x_dec=None, x_mark_dec=None):
        ...
        # Log transform dynamic inputs
        eps = 1e-6
        x_enc[..., 0:1] = torch.log(x_enc[..., 0:1] + eps)
        x_dec = torch.log(x_dec + eps)
        ...
        # 3洪涝数据嵌入并选择嵌入前与降雨融合或嵌入后融合
        fuse_before_embedding=False
        if fuse_before_embedding:
            rain_input=rain_input.view(batch_size, 7, seq_len, -1)
            ...
            enc_out_rain=enc_out_rain.view(batch_size, 7, seq_len, -1)
            flood_rain_effect= self.compute_rainfall_influence(enc_out_rain, adj)
            flood_input = dynamic_feature[:, 7:, :, :]
            flood_input=flood_input.reshape(batch_size * 56, seq_len, 1)  # [B*56, T, 1]
            x_mark_enc_flood=x_mark_enc.unsqueeze(1).repeat(1, 56, 1, 1)
            x_mark_enc_flood= x_mark_enc_flood.reshape(batch_size * 56, seq_len, 4)
            flood_embedding = self.enc_embedding_flood(flood_input, x_mark_enc_flood)  # [batch_size*56, seq_len, d_model]
            flood_embedding=flood_embedding.view(batch_size, 56, seq_len, -1)
            flood_embedding=flood_embedding+flood_rain_effect
        ...
        reprs = flood_pred.unsqueeze(-1)   # [B, 56, seq_len, d_model]
      
        # Exponentiate predictions to invert log-transform
        pred_all = torch.exp(pred_all)
        return pred_all, reprs
