...
class Model(nn.Module):
    def __init__(self,configs):
        ...
        # 新增未来预测的降雨 embedding 和 encoder 模块
    ...
    def forward(self, x_enc, future_rain, x_mark_enc=None, x_dec=None, x_mark_dec=None):
        ...
        enc_out_rain_effect = self.compute_rainfall_influence(
            enc_out_rain.reshape(batch_size,7,seq_len,-1),
            adj, self.alpha2, self.beta2
        )
        # 4. 静态信息中嵌入
        projected_static = []
        for i in range(5):
        ...
        flood_encoded=flood_encoded+enc_out_rain_effect
        ...
        if x_mark_enc is None:
            dec_out_rain = self.dec_embedding_rain(x_dec_rain, None)
            dec_out_flood = self.dec_embedding_flood(x_dec_flood, None)
        else:
        ...
        # 对 rain_pred 进行 embedding 和 encode
        ...
        flood_encoded=flood_encoded.reshape(batch_size * 56, seq_len, -1)
        # print("dec_f.shape",dec_f.shape)
        # print("flood_encoded.shape",flood_encoded.shape)
        # print("static_s_dec",static_s_dec.shape)
        # print("future_rain_effect",future_rain_effect.shape)
        for layer in self.flood_decoder_layers_future_rain:
            dec_f = layer(dec_f, flood_encoded, static_s_dec,future_rain_effect, self_mask=None, cross_mask=None)
