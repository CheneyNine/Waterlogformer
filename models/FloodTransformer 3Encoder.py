...
class SelectorLayer(nn.Module):
    """
    Cross-attention based selector: q from temporal, k/v from spatial/flood.
    """
    ...
    def forward(self, q, k, v):
        ...
        q2 = self.norm1(q + attn_out)
        ff_out = self.ff(q2)
        out = self.norm2(q2 + ff_out)
        return out

import torch

# === MaskObject: Wrapper for boolean mask tensor ===
class MaskObject:
    """
    Wrapper for boolean mask tensor to comply with SelfAttention_Family.attn_mask interface.
    """
    def __init__(self, mask):
        self.mask = mask

# === Multi-Scale Temporal Layer ===
class MultiScaleTemporalLayer(nn.Module):
    """
    Multi-scale temporal encoder: parallel global and local self-attention.
    """
    def __init__(self, d_model, n_heads, d_ff, window_size, dropout, factor):
        super().__init__()
    ...
    def forward(self, x, attn_mask=None):
        ...
        # Global path
        g, _ = self.global_layer(x, attn_mask=None)
        # Build local attention mask: mask True where distance > window_size
        T = x.size(1)
        idx = torch.arange(T, device=x.device)
        local_mask = (idx.unsqueeze(1) - idx.unsqueeze(0)).abs() > self.window_size
        # Wrap mask in object expected by SelfAttention_Family
        mask_obj = MaskObject(local_mask)
        # Local path with wrapped mask
        l, _ = self.local_layer(x, attn_mask=mask_obj)
        # Fuse
        cat = torch.cat([g, l], dim=-1)  # [batch, seq_len, 2*d_model]
        out = self.norm(x + self.ffn(cat))
        return out, None
import numpy as np
...
class Model(nn.Module):
    def __init__(self,configs):
        ...
        # === 新增：分别捕捉时间和空间特征的 Encoder layers ===
        # 假设 configs.d_model=512
        self.flood_input_proj = nn.Linear(1, configs.d_model)
        self.flood_input_proj2 = nn.Linear(self.static_d+configs.d_model ,configs.d_model )

        # === 新增：多尺度时间编码层 (全局 + 局部窗口) ===
        window_size = getattr(configs, 'local_window', 5)
        self.temporal_flood_encoder_layers = nn.ModuleList([
            MultiScaleTemporalLayer(
                configs.d_model, configs.n_heads, configs.d_ff,
                window_size, configs.dropout, configs.factor
            )
            for _ in range(configs.e_layers)
        ])
        self.spatial_flood_encoder_layers = nn.ModuleList([
            EncoderLayer(
                AttentionLayer(
        ...
        # === 新增：Selector 选择器层，用于融合 temp_x 和 enc_out_flood ===
        selector_count = getattr(configs, 'selector_layers', configs.e_layers)
        self.selector_layers = nn.ModuleList([
            SelectorLayer(configs.d_model, configs.n_heads, configs.d_ff, configs.dropout)
    ...
    def forward(self, x_enc, future_rain, x_mark_enc=None, x_dec=None, x_mark_dec=None):
        ...
        flood_token_seq = torch.cat([static_info_embedding, flood_embedding], dim=3)  # [B, 56, T, 512+128]
        
        
        
        # === 并行 Flood 编码：时间 & 空间 ===
        # Temporal branch
        temp_x = self.flood_input_proj(flood_input)  
        for layer in self.temporal_flood_encoder_layers:
            temp_x, _ = layer(temp_x, attn_mask=None)
        # Spatial branch
        temp_y = self.flood_input_proj2(flood_token_seq)  
        spatial_input = temp_y.permute(0, 2, 1, 3).reshape(batch_size * (seq_len), 56, -1)
        spatial_x = spatial_input
        for layer in self.spatial_flood_encoder_layers:
            spatial_x, _ = layer(spatial_x, attn_mask=None)
        spatial_x = spatial_x.reshape(batch_size, seq_len, 56, -1)\
                              .permute(0, 2, 1, 3)\
                              .reshape(batch_size * 56, seq_len, -1)
        # Original Branch
        flood_x = self.flood_input_proj(flood_input) 

        # === Selector: 使用 Transformer encoder cross-attention 融合 temp_x (q) 和 enc_out_flood (k,v) ===
        # 转置至[seq, batch, d_model]
        q = temp_x.permute(1, 0, 2)
        k = flood_x.permute(1, 0, 2)
        v = spatial_x.permute(1, 0, 2)
        sel = q
        for layer in self.selector_layers:
            sel = layer(sel, k, v)
        # 转回 [batch, seq, d_model]
        enc_out_flood = sel.permute(1, 0, 2)
        ...
        flood_encoded = enc_out_flood.view(batch_size, 56, seq_len, -1)  # [B, 56, T, d_model]
