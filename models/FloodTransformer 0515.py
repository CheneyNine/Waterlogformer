...
class Model(nn.Module):
    def __init__(self,configs):
        ...
        self.use_future_rain=configs.use_future_rain
        self.use_real_rain=configs.use_real_rain
        self.number_fuse=configs.number_fuse
        self.embed_fuse=configs.embed_fuse
    ...
    def forward(self, x_enc, future_rain, x_mark_enc=None, x_dec=None, x_mark_dec=None):
        ...
        static_s_dec = static_node_feat.unsqueeze(2).expand(batch_size, 56, self.pred_len+self.label_len, self.static_d)
        static_s_dec = static_s_dec.reshape(batch_size * 56, self.pred_len+self.label_len, self.static_d) 


        if self.use_future_rain:
            if self.use_real_rain:
                rain_pred=future_rain
            if self.number_fuse:
                #数值融合
                rain_pred_numerial_effect=self.compute_rainfall_influence(rain_pred.reshape(batch_size,7,seq_len,-1),adj, self.alpha3, self.beta3)
                x_mark_future_rain = x_mark_dec[:, -self.pred_len:, :].unsqueeze(1).repeat(1, 56, 1, 1)
                x_mark_future_rain = x_mark_future_rain.reshape(batch_size * 56, self.pred_len, 4)
                rain_pred_numerial_effect=rain_pred_numerial_effect.reshape(batch_size * 56, seq_len, -1)
                future_rain_embed = self.future_rain_embedding(rain_pred_numerial_effect, x_mark_future_rain)
                static_s_rain = torch.zeros_like(future_rain_embed[..., :self.static_d])
                xr = future_rain_embed
                for layer in self.future_rain_encoder_layers:
                    xr, _ = layer(xr, static_s_rain)
                future_rain_embed = xr  # [batch_size*7, seq_len, d_model]
        

            elif self.embed_fuse:
                # 嵌入融合
                ...
                future_rain_embed_effect=self.compute_rainfall_influence(future_rain_embed.reshape(batch_size,7,seq_len,-1),adj, self.alpha3, self.beta3)
                xr = future_rain_embed_effect.reshape(batch_size*56,seq_len,-1)
                static_s_rain = torch.zeros_like(xr[..., :self.static_d])
                for layer in self.future_rain_encoder_layers:
                    xr, _ = layer(xr, static_s_rain)
                future_rain_embed = xr  # [batch_size*7, seq_len, d_model]
                # future_rain_encoded, _ = self.future_rain_encoder(future_rain_embed_effect)
                ...
                future_rain_embed=self.compute_rainfall_influence(future_rain_embed.reshape(batch_size,7,seq_len,-1),adj, self.alpha3, self.beta3)
                future_rain_embed=future_rain_embed.reshape(batch_size*56,seq_len,-1)
            ...
            for layer in self.flood_decoder_layers_future_rain:
                dec_f = layer(dec_f, future_rain_embed, static_s_dec,enc_out_flood, self_mask=None, cross_mask=None)
        else:
            # print(future_rain.shape)torch.Size([4, 63, 24])
        ...
        return flood_pred, total_contrastive_loss
    ...
    def compute_rainfall_influence(self, rain_encoded, adj_matrix, alpha=None, beta=None):
        ...
        if self.without_propagation:
            flood_rain_effect=rain_to_flood
        else:
            # 初始化参数
