...
class Model(nn.Module):
    ...
    def forward(self, x_enc, future_rain, x_mark_enc=None, x_dec=None, x_mark_dec=None, mask=None):
        ...
        # 5. 拼接洪涝数据和静态数据，并 encoder
        # flood_embedding: [B, 56, T, d_model], static_info_embedding: [B, 56, T, static_d]
        flood_x = torch.cat([flood_embedding, static_info_embedding], dim=-1)  # [B, 56, T, d_model+static_d]
        flood_x = self.flood_static_fusion(flood_x)  # [B, 56, T, d_model]
        flood_x = flood_x.reshape(batch_size * 56, seq_len, -1)  # [B*56, T, d_model]
        enc_out_flood, _ = self.flood_encoder(flood_x)  # [B*56, T, d_model]
        # 保留全部时间步或仅最后一步
        ...
        # 4. 相加后解码
        flood_context = future_rain_for_flood + h_out_flood  # [B*56, pred_len, d_model]
        flood_dec_input = torch.zeros(batch_size * 56, self.pred_len, self.embed_dim, device=x_enc.device)  # [B*56, pred_len, d_model]
        flood_dec_out = self.flood_decoder(flood_dec_input, flood_context, x_mask=None, cross_mask=None)  # [B*56, pred_len]
