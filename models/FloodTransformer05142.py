...
class Model(nn.Module):
    ...
    def forward(self, x_enc, future_rain, x_mark_enc=None, x_dec=None, x_mark_dec=None):
        ...
        # 保留全部时间步或仅最后一步
        rain_encoded = enc_out_rain.reshape(batch_size, 7, seq_len, -1)  # [B, 7, T, d_model]
        # 洪水flood encoder输出的CLS token和flood编码
        flood_encoded = enc_out_flood[:, 1:, :].reshape(batch_size, 56, seq_len, -1)  # [B, 56, T, d_model]
        flood_rain_effect = self.compute_rainfall_influence(rain_encoded, adj, self.alpha2, self.beta2)
        flood_encoded=flood_encoded+flood_rain_effect
        ...
        for layer in self.flood_decoder_layers:
            dec_f = layer(dec_f, flood_encoded, static_s_dec, self_mask=None, cross_mask=None)
