...
class Dataset_Custom_Baseline(Dataset):
    ...
    def __read_data__(self):
        ...
        # Exclude rain columns
        # self.rain_cols = [col for col in cols if 'rain' in col]
        self.all_cols = self.flood_cols  # Only flood-related columns
        ...
        if self.scale:
            train_data = df_data[self.all_cols].values[border1s[0]:border2s[0]]
            self.scaler.fit(train_data)
            data = self.scaler.transform(df_data[self.all_cols].values)
        else:
...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        self.data_y = data[border1:border2]
        self.data_stamp = data_stamp
        # build list of valid window start indices where span ≤ 1 day
        max_start = len(self.date_series) - self.seq_len - self.pred_len + 1
        self.valid_indices = []
        for i in range(max_start):
            window_dates = self.date_series[i : i + self.seq_len]
            if window_dates.max() - window_dates.min() <= timedelta(days=1):
                self.valid_indices.append(i)


    def __getitem__(self, index):
        # map to a valid window index
